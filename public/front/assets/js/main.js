		...
		$("#slider-range").slider({
			...
			slide: function(event, ui) {
			 ...
			 change: function(event, ui) {
				onRangeChange(ui.values[0], ui.values[1]);
			 }
		});
		$("#slider-range").slider("option", "min",parseInt($('#min-value').val()));
		$("#slider-range").slider("option", "max", parseInt($('#max-value').val()));
		$("#slider-range").slider("option", "values",[parseInt($('#min-value').val()), parseInt($('#max-value').val())]);
		$("#amount").val("$" + $("#slider-range").slider("values", 0) +
		" - $" + $("#slider-range").slider("values", 1));
	}
	price_slider();
	function onRangeChange(min, max) {
		console.log("Range changed:", min, max);
		$('.filter_product').trigger('click');
